...
def run_cycle(cfg: dict, wallet: PaperWallet | None = None) -> PaperWallet:
    ...
    wallet = wallet or PaperWallet(initial_cash=cfg['initial_cash_usdt'], fee_pct=cfg['fee_pct'])
    wallet.next_cycle()
    cycle = wallet._cycle
    risk = RiskManager(
        ...
        if use_scoring and signal == 'buy' and not has_position:
            try:
                ...
                if not should_execute(_score, min_confidence=0.4, min_risk=0.5):
                    print(
                        f'{symbol}: BUY rejeitado pelo scoring (conf={_score.confidence:.2f} risk={_score.risk_score:.2f})'
                    )
                    signal = 'hold'
            except Exception as e:
...
def main():
    ...
    ap.add_argument(
        '--strategy',
        choices=['grid', 'grid_dynamic', 'combined', 'default', 'agent_desk'],
        default=None,
    ...
    print(
        f'=== Paper Bot (spot) | estrategia={cfg.get("strategy", STRATEGY_DEFAULT)} | caixa ${wallet.cash:.2f} ==='
    )
