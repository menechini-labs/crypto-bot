...
from core.strategy_registry.base import BaseStrategy
...
def _strategy_signal(
    strategy_name: str, closes: list[float], grid_levels=None, has_position=False
) -> str:
...
def run_backtest(
    ...
    logger.info(
        'run_backtest symbol=%s strategy=%s n=%d scoring=%s obj=%s',
        symbol,
        strategy_name,
        len(closes),
        use_scoring,
    ...
    if strategy_name == 'grid':
        ...
        grid_levels = build_dynamic_grid(
            center=initial_center, step=max(initial_center * 0.01, 1e-8), n=11
        )
        ...
        if symbol in wallet.positions and entry:
            ...
            if use_scoring and score_signal is not None and should_execute is not None:
                ...
                if not should_execute(_score, min_confidence=0.4, min_risk=0.5):
                    logger.debug(
                        'buy rejeitado pelo scoring (conf=%.2f risk=%.2f)',
                        _score.confidence,
                        _score.risk_score,
                    )
                    continue
