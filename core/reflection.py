...
def reflect_trades(
    ...
    durations = [_safe(t.get('duration_min')) for t in trades if _safe(t.get('duration_min')) > 0]
    ...
    buy_win_rate = sum(1 for t in buys if _safe(t.get('pnl')) > 0) / len(buys) if buys else 0.0
    sell_win_rate = sum(1 for t in sells if _safe(t.get('pnl')) > 0) / len(sells) if sells else 0.0

    insights: list[str] = []

    if win_rate >= 0.6:
        insights.append(f'Win rate {win_rate:.0%} — saudavel, acima de 60%.')
    elif win_rate >= 0.45:
        insights.append(f'Win rate {win_rate:.0%} — mediano. Avaliar relacao win/loss.')
    else:
        insights.append(f'Win rate {win_rate:.0%} — baixo. Revisar setup de entrada.')
    ...
    if avg_winner > 0 and avg_loser < 0:
        ...
        if ratio > 2:
            ...
            insights.append(f'Relacao ganho/perda {ratio:.1f}x — equilibrado.')
        else:
            insights.append(
    ...
    if max_consecutive_losses >= 5:
        ...
        insights.append(f'{max_consecutive_losses} perdas consecutivas — aceitavel, mas monitorar.')
    else:
        insights.append(f'Maximo de perdas consecutivas: {max_consecutive_losses} — controle ok.')
    ...
    if avg_duration > 0:
        if avg_duration > 1440:
            ...
            insights.append(f'Duracao media {avg_duration:.0f}min — adequada.')
    ...
    if losers and durations:
        loser_durations = [
            _safe(t.get('duration_min')) for t in losers if _safe(t.get('duration_min')) > 0
        ]
        if loser_durations:
            ...
            patterns.append(
                {
                    ...
                    'top3_pnl_share': round(top3 / total_pnl, 3) if total_pnl != 0 else 0,
                }
            )
    ...
    for r, pnls_list in regimes_found.items():
        regime_perf[r] = {
            ...
            'win_rate': (
                round(sum(1 for p in pnls_list if p > 0) / len(pnls_list), 3) if pnls_list else 0
            ),
        }
...
def _generate_recommendations(insights: list[str], patterns: list[dict[str, Any]]) -> list[str]:
    recs: list[str] = []
    ...
    for ins in insights:
        if 'perda' in ins.lower() and 'consecutivo' in ins.lower() and '5' in ins:
            recs.append('Adicionar circuit breaker: pausar trading apos 3 perdas consecutivas.')
    ...
    if has_recent_decline and has_regime_weakness:
        recs.append('POSSIVEL MUDANCA DE REGIME. Reavaliar seletor de estrategia.')
