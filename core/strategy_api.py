...
from core.scoring import detect_regime as _detect_regime
from core.scoring import score_signal as _score_signal
from core.scoring import should_execute
...
# ---------------------------------------------------------------------------
# Stateful 1m signal engine (reuses core.market_ws.WebSocketMarket)
# ---------------------------------------------------------------------------
# Mapa de engines ativos: key (symbol|strategy_name) -> dict com lock + last_bar
_1M_ENGINES: dict[str, dict] = {}


def start_1m_signal_engine(symbol: str, strategy_name: str, on_signal) -> dict:
    """Inicia um motor de sinais 1m que dispara NO MÁXIMO UMA VEZ por barra fechada.

    Reusa WebSocketMarket (Binance 1m kline) via candle_listener — NÃO cria novo
    polling loop. `on_signal(signal_dict, ctx)` é chamado quando uma barra 1m fecha.
    Retorna o handle do engine (com .stop() para encerrar).
    """
    import asyncio
    ...
    if key in _1M_ENGINES:
        return _1M_ENGINES[key]

    strategy_cls = _get_strategy(strategy_name)
    ...
    engine_state: dict[str, Any] = {
        ...
        'on_signal': on_signal,
        'strategy': strategy,
        'last_bar_ts': None,  # timestamp da última barra processada (estado)
        'running': True,
        'market': None,
        'task': None,
    }

    def _listener(candle: dict) -> None:
        """Disparado pelo WebSocketMarket para cada vela. Só age em barras FECHADAS
        e no máximo uma vez por timestamp (estado anti-duplicação)."""
        if not candle.get('is_closed'):
            return
        ts = candle.get('ts')
        if ts is None:
            return
        if engine_state['last_bar_ts'] == ts:
            return  # já processamos essa barra
        engine_state['last_bar_ts'] = ts
        cache = market.cache
        highs = [c['high'] for c in cache]
        lows = [c['low'] for c in cache]
        vols = [c['volume'] for c in cache]
        ...
        try:
            sig = strategy.decide(closes, has_position=False, ctx=ctx)
        except Exception as exc:  # estratégia não deve derrubar o loop
            logger.warning('1m engine decide error (%s): %s', strategy_name, exc)
            return
        try:
            on_signal(sig, ctx)
        except Exception as exc:
            logger.warning('1m engine on_signal error: %s', exc)

    market = WebSocketMarket(symbol=symbol, interval='1m', cache_limit=200, strategy_name=None)
    market.candle_listener = _listener
    engine_state['market'] = market

    def stop() -> None:
        engine_state['running'] = False
        try:
            loop = asyncio.get_event_loop()
            loop.run_until_complete(market.stop())
        except Exception:
            try:
                lp = asyncio.new_event_loop()
                lp.run_until_complete(market.stop())
                lp.close()
            except Exception:
                pass
        _1M_ENGINES.pop(key, None)

    engine_state['stop'] = stop
    _1M_ENGINES[key] = engine_state

    # Inicia a conexão WS em background (reusa loop asyncio existente)
    try:
        loop = asyncio.get_event_loop()
        engine_state['task'] = loop.create_task(market.connect())
    except RuntimeError:
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        engine_state['task'] = loop.create_task(market.connect())

    logger.info('1m signal engine started: %s/%s', symbol, strategy_name)
    return engine_state
...
async def _warm_backtest_cache() -> None:
    ...
    for name in reg:
        ...
        try:
            result = _run(
                closes, config, symbol='BTCUSDT', strategy_name=name, use_scoring=(name == 'llm')
            )
...
async def list_strategies(
    ...
    if minPnl is not None:
        results = [
            s for s in results if s['netProfitPct'] is not None and s['netProfitPct'] >= minPnl
        ]
    if maxDd is not None:
        results = [
            s for s in results if s['maxDrawdownPct'] is not None and s['maxDrawdownPct'] <= maxDd
        ]
    if minSharpe is not None:
        results = [
            s for s in results if s['sharpeRatio'] is not None and s['sharpeRatio'] >= minSharpe
        ]
    if author:
...
async def api_score(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    logger.info(
        'api_score signal=%s closes=%d', payload.get('signal'), len(payload.get('closes', []))
    )
    from core.scoring import explain_score, score_signal
...
async def market_closes(
    ...
    try:
        ...
        raise HTTPException(502, f'Falha ao buscar klines: {exc}') from exc
    return {
        'status': 'ok',
        'symbol': symbol,
        'timeframe': timeframe,
...
async def api_indicators(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(500, f'Erro ao calcular indicadores: {exc}') from exc
    return {
        'status': 'ok',
        'symbol': payload.get('symbol', 'BTCUSDT'),
...
async def api_signals(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(closes, list) or len(closes) < 30:
        try:
            ...
            raise HTTPException(502, f'Falha ao buscar closes: {exc}') from exc
    c = [float(x) for x in closes]
...
def _agent_loop_worker() -> None:
    ...
    try:
        ...
        logging.exception('agent_loop worker died')
...
def _agent_loop_1m_on_signal(sig: dict, ctx: dict) -> None:
    """Recebe sinais do motor 1m e executa via PaperEngine (guarda REAL preservada)."""
    global _AGENT_LOOP_CFG
    try:
        from core import agent_desk as _ad
        from core import paper_engine as _pe
        ...
        if not cfg.get('auto_trade', True):
            logging.info(
                'agent_loop(1m) analise-only (%s conf %.2f) -- auto_trade off', verdict, conf
            )
            return
        engine = _pe.get_engine()
        snap = engine.snapshot()
        avail = snap.get('available_cash', snap.get('cash', 0.0))
        symbol = _AGENT_LOOP_TEAM_SYMBOL or sig.get('symbol', 'BTCUSDT')
        qty = _ad._compute_qty(symbol, conf, avail)
        if qty <= 0:
            return
        tp_pct = cfg['tp_pct']
        ...
        if target_price:
            ...
            if price > 0:
                tp_pct = float(target_price) / price - 1.0
        sl_pct = 0.0 if cfg.get('lock_stop') else cfg['sl_pct']
        order = _pe.Order(
            symbol=symbol,
            side=verdict,
            ...
            tp_pct=tp_pct,
            trailing_pct=cfg['trailing_pct'],
            reason=f'1m engine {sig.get("reason", "")}',
            advisory=sig.get('reason', ''),
        )
        res = engine.submit(order)
        logging.info('agent_loop(1m) %s %s -> ok=%s', verdict, qty, res.get('ok'))
    except Exception as e:
        logging.warning('agent_loop_1m_on_signal error: %s', e)
...
async def api_agents_loop_start(payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    _AGENT_LOOP_CFG = {
        ...
        if p.get('target_price') not in (None, '', 0)
        else None,
        'auto_trade': bool(p.get('auto_trade', True)),
        'lock_stop': bool(p.get('lock_stop', False)),
    }
    # Day-trade templates: se o preset nomeia uma estratégia 1m, liga o motor de
    # sinais 1m (reuso de WebSocketMarket). O loop worker continua p/ análise/1h.
    global _AGENT_LOOP_1M_ENGINE
    _AGENT_LOOP_1M_ENGINE = None
    try:
        from core.swarm_presets import get_preset as _gp

        preset = _gp(_AGENT_LOOP_TEAM)
        strat = preset.get('behaviors', {}).get('strategy') if preset else None
        if strat:
            _AGENT_LOOP_1M_ENGINE = start_1m_signal_engine(
                _AGENT_LOOP_TEAM_SYMBOL, strat, _agent_loop_1m_on_signal
            )
            logging.info(
                "loop start: 1m signal engine ativo para '%s' (strat=%s)", _AGENT_LOOP_TEAM, strat
            )
    except Exception as e:
        logging.warning('loop start: falha ao ligar 1m engine: %s', e)
...
async def api_agents_loop_stop() -> dict[str, Any]:
    ...
    global _AGENT_LOOP_THREAD, _AGENT_LOOP_1M_ENGINE
    ...
    # Encerra o motor 1m se estiver ativo (day-trade templates)
    if _AGENT_LOOP_1M_ENGINE is not None:
        try:
            _AGENT_LOOP_1M_ENGINE['stop']()
        except Exception as e:
            logging.warning('loop stop: erro ao parar 1m engine: %s', e)
        _AGENT_LOOP_1M_ENGINE = None
...
async def api_news(
    ...
    sources: str | None = Query(
        None, description='comma-separated feed ids (coindesk,cointelegraph)'
    ),
) -> dict[str, Any]:
...
@app.post('/api/orders')
async def api_submit_order(payload: dict[str, Any]) -> Response:
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail='qty invalido') from err
    if qty <= 0:
...
async def download_project() -> FileResponse:
    ...
    return FileResponse(tmp_zip, media_type='application/zip', filename='crypto-bot-project.zip')
...
async def serve_index():
    ...
    return Response(
        status_code=404, content='Dashboard nao compilado. Rode `npm run build` em dashboard/'
    )
