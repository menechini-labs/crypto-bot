...
import { ApiError, apiGet, apiPost, parseNum, parsePct } from "./api";
  ...
  return (
    <div className={`agent-card agent-card--${tone}`}>
      <div className="agent-card__head">
        ...
        <span className={`agent-card__verdict agent-card__verdict--${tone}`}>
...
export default function AgentDesk() {
    ...
    try {
      ...
      if (s.config) {
        setCfg((c) => ({
          ...
          trailing_pct:
            s.config!.trailing_pct != null ? String(s.config!.trailing_pct * 100) : c.trailing_pct,
  ...
  async function toggleLoop() {
    ...
    try {
      ...
      if (
        !loopRunning &&
        (sl === null || tp === null || tr === null || (cfg.target_price !== "" && target === null))
      ) {
  ...
  async function execute() {
    ...
    try {
      const data = await apiPost<{
        ok: boolean;
        error?: string;
        execution?: { result?: { order?: { side?: string; qty?: number | string } } };
      }>("/api/agents/execute", { cycle_id: cycle.cycle_id, team: cycle.team ?? null });
      if (data.ok) {
  ...
  const canExecute =
    isReal && dec && (dec.verdict === "buy" || dec.verdict === "sell") && dec.confidence >= 0.5;
  const decTone = dec?.verdict === "buy" ? "ok" : dec?.verdict === "sell" ? "rej" : "warn";
    ...
    <div className="agent-desk">
      <div className="agent-desk__head">
        ...
        <div className={`agent-desk__decision agent-desk__decision--${decTone}`}>
          ...
          <span className="agent-desk__decision-conf">
        ...
        <button
          ...
          title={
            mode !== "real"
              ? "Loop requer modo REAL"
              : loopRunning
                ? "Parar loop automático"
      ...
      <div className="agent-desk__preset">
        ...
        <span className="muted" style={{ fontSize: 11 }}>
          {presets.find((p) => p.name === template)?.description ??
            "Selecione um template de day-trade para o PLAY usar estratégia 1m."}
        </span>
      </div>
      ...
      <div className="agent-desk__playcfg">
        ...
        <div className="agent-desk__playcfg-grid">
          <label>
            ...
            <input
              ...
              onChange={(e) => setCfg({ ...cfg, sl_pct: e.target.value })}
            />
          </label>
          <label>
            <span>TP %</span>
            <input
              value={cfg.tp_pct}
              disabled={loopRunning}
              onChange={(e) => setCfg({ ...cfg, tp_pct: e.target.value })}
            />
          </label>
          <label>
            <span>Trailing %</span>
            <input
              value={cfg.trailing_pct}
              disabled={loopRunning}
              onChange={(e) => setCfg({ ...cfg, trailing_pct: e.target.value })}
            />
          </label>
          <label>
            <span>Meta ($)</span>
            <input
              value={cfg.target_price}
              disabled={loopRunning}
              placeholder="ex: 64000"
              onChange={(e) => setCfg({ ...cfg, target_price: e.target.value })}
            />
          </label>
          <label className="agent-desk__toggle">
            <input
              type="checkbox"
              checked={cfg.auto_trade}
              disabled={loopRunning}
              onChange={(e) => setCfg({ ...cfg, auto_trade: e.target.checked })}
            />
          ...
          <label className="agent-desk__toggle">
            <input
              type="checkbox"
              checked={cfg.lock_stop}
              disabled={loopRunning}
              onChange={(e) => setCfg({ ...cfg, lock_stop: e.target.checked })}
            />
            <span>Lock Stop</span>
          </label>
        </div>
        <span className="muted" style={{ fontSize: 11 }}>
          {cfg.auto_trade
            ? "Auto: executa ordens quando conf≥0.5."
            : "Análise only: PLAY não executa — use Executar ordem."}
      ...
      <div className="agent-desk__exec">
        {isReal ? (
          <button
            ...
            title={
              canExecute ? "Executar ordem via PaperEngine" : "Apenas REAL + buy/sell conf>=0.5"
            }
          >
      ...
      {reflOpen && (
        <div className="agent-desk__reflections">
          <div className="agent-desk__reflections-head">
            ...
            <button type="button" className="btn btn--small" onClick={() => setReflOpen(false)}>
              Fechar
            </button>
          </div>
                ...
                <div key={i} className="reflection-card">
                  ...
                  {(r.recommendations?.length ?? 0) > 0 && (
                    <div className="reflection-card__recs">
                      ...
                      <ul>
                        {(r.recommendations ?? []).map((rec: string, k: number) => (
                          <li key={k}>{rec}</li>
                        ))}
                      </ul>
                    </div>
