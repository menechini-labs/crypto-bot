...
import { runBacktest, fetchBacktestReport, triggerBacktestReflection } from "./api";
import type { BacktestParams, BacktestReport as ReportData, AnalysisResult } from "./types";
    ...
    <article className="report-view">
      ...
      {reflection && (
        <section className="reflection-panel">
          ...
          <div className="reflection-metrics">
            ...
            <div><small>Max Perdas Consecutivas</small><span>{reflection.metrics ? String((reflection.metrics as Record<string, number>).max_consecutive_losses ?? 0) : "—"}</span></div>
          </div>
          {Array.isArray(reflection.insights) && reflection.insights.length > 0 && (
            <ul className="reflection-insights">
              {(reflection.insights as string[]).slice(0, 5).map((i, idx) => <li key={idx}>{i}</li>)}
            </ul>
          )}
          {Array.isArray(reflection.recommendations) && reflection.recommendations.length > 0 && (
            <div className="reflection-recs">
              <h4>Recomendações</h4>
              <ul>
                {(reflection.recommendations as string[]).slice(0, 3).map((r, idx) => <li key={idx}>{r}</li>)}
              </ul>
            </div>
          )}
        </section>
      ...
      <section className="report-kpis" aria-label="Métricas principais">
        <div className="kpi-card">
          ...
          <span className={`kpi-value ${pnlClass}`}>{report.pnl_pct >= 0 ? "+" : ""}{report.pnl_pct.toFixed(2)}%</span>
        </div>
        <div className="kpi-card">
      ...
      {report.equity_curve && report.equity_curve.length > 0 && (
        <section className="report-chart" aria-label="Curva de equity">
          ...
          <p className="chart-caption">
            {report.equity_curve.length} candles · Início: ${report.equity.toFixed(2)} · Fim: ${report.final_equity.toFixed(2)}
          </p>
        </section>
      )}
      ...
      <section className="report-details" aria-label="Detalhes">
        ...
        <dl className="details-grid">
          <div><dt>Símbolo</dt><dd>{report.symbol}</dd></div>
          <div><dt>Estratégia</dt><dd>{report.strategy}</dd></div>
          <div><dt>Regime</dt><dd>{report.regime}</dd></div>
          <div><dt>Candles</dt><dd>{report.candles}</dd></div>
          <div><dt>Seed</dt><dd>{report.seed}</dd></div>
          <div><dt>Capital Inicial</dt><dd>${report.equity.toFixed(2)}</dd></div>
          <div><dt>Capital Final</dt><dd>${report.final_equity.toFixed(2)}</dd></div>
          <div><dt>PnL Absoluto</dt><dd className={pnlClass}>${report.pnl.toFixed(2)}</dd></div>
          <div><dt>Profit Factor</dt><dd>{report.profit_factor?.toFixed(2) ?? "N/A"}</dd></div>
          <div><dt>Sortino</dt><dd>{report.sortino?.toFixed(2) ?? "N/A"}</dd></div>
          <div><dt>Calmar</dt><dd>{report.calmar?.toFixed(2) ?? "N/A"}</dd></div>
        </dl>
      </section>
      ...
      {report.trades_list && report.trades_list.length > 0 && (
        <section className="report-trades" aria-label="Lista de trades">
          ...
          <div className="trades-table-wrap">
            ...
            {report.trades_list.length > 50 && <p className="trades-truncated">Mostrando 50 de {report.trades_list.length} trades</p>}
          </div>
        </section>
      )}
  ...
  return (
    <form className="backtest-form-inline" onSubmit={submit}>
      ...
      <div className="form-row">
        ...
        <label>
          <span>Regime</span>
          <select value={params.regime} onChange={(e) => set("regime", e.target.value as BacktestParams["regime"])}>
        ...
        <label>
          <span>Candles</span>
          <input type="number" min={50} max={2000} value={params.n} onChange={(e) => set("n", Number(e.target.value))} />
        </label>
        <label>
          <span>Seed</span>
          <input type="number" min={0} max={1_000_000} value={params.seed} onChange={(e) => set("seed", Number(e.target.value))} />
        </label>
      </div>
