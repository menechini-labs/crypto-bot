...
class TestRiskAgent:
    ...
    @patch(
        'core.agent_desk.config_loader.load_config',
        return_value={'risk': {'max_drawdown_pct': 0.40}},
    )
...
class TestAgentVerdictSerialization:
    def test_to_dict(self):
        v = AgentVerdict('TestAgent', 'test', 'ok', 0.85, 'Tudo certo.', {'foo': 42})
        d = v.to_dict()
...
class TestExecuteCycle:
    ...
    def test_compute_qty_rounded_to_step(self):
        ...
        px = float(
            __import__('core.market', fromlist=['fetch_ticker']).fetch_ticker('BTCUSDT')['price']
        )
