...
def test_paper_engine_closed_trades():
    ...
    try:
        ...
        assert engine.submit(
            pe.Order(symbol='BTCUSDT', side='buy', qty=0.01, sl_pct=0.02, tp_pct=0.05, reason='t')
        )['ok']
        assert engine.submit(pe.Order(symbol='BTCUSDT', side='sell', qty=0.01, reason='close'))[
            'ok'
        ]
